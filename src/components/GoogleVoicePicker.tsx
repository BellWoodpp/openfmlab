  ...
  useEffect(() => {
    ...
    fetch(url.toString(), { signal: controller.signal })
      ...
      .then((data) => {
        ...
        const picked = pickDefaultVoice(list);
    ...
    return () => controller.abort();
  }, [enabled, resolvedLang, debouncedQuery]);
  ...
  return (
    <div className="mt-3 flex flex-col h-full min-h-0">
      <div className="flex flex-col gap-3 shrink-0">
        ...
        >
          <span className="relative inline-flex items-center">
            Auto pick
            {isAutoMode ? (
              <span className="absolute -top-2 -right-3 inline-flex h-4 w-4 items-center justify-center rounded-full bg-primary text-primary-foreground ring-2 ring-background">
      ...
      <div className="mt-2 text-xs text-muted-foreground shrink-0">
        Current voice: <span className="font-mono text-foreground">{currentVoice}</span>
      ...
      <div className="mt-1 text-xs text-muted-foreground shrink-0">
        Found <span className="font-mono text-foreground">{voices.length}</span> voices.{" "}
        {tierCounts.length > 0 && (
          <>
            Tiers:{" "}
            {tierCounts
      ...
      <div className="mt-3 flex-1 overflow-y-auto rounded-xl border border-border bg-muted/20 min-h-0 max-h-[420px] shadow-inner">
        {loading ? (
          <div className="p-4 text-sm text-muted-foreground text-center">Loading voices…</div>
        ) : error ? (
